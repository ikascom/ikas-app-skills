...
| `[mcp]` | Verified against the live Admin API schema through the ikas admin MCP (`https://api.myikas.com/api/v2/admin/mcp`, `introspect`), 2026-09-11 |
...
- **SHOULD** `[mcp]` compare `getAuthorizedApp.scope` with the scope the code requested and log a mismatch — this is the only runtime check for the Partner-panel ↔ code drift in §2.1. Missing → **Bilgi**.
...
- `[mcp]` `deleteWebhook(scopes: [[String!]]!)` removes registrations by scope — the uninstall cleanup call when the app registered data webhooks.
...
- `[mcp]` `MerchantAppPayment.status` ∈ `PAID | PAYMENT_FAILED | WAITING_FOR_PAYMENT`; `type` ∈ `ONE_TIME | SUBSCRIPTION | WALLET_ACTION`; `prices[] { period: MONTHLY|YEARLY|ONE_TIME, price }`; `storeAppListingSubscriptionKey` nullable. `listMerchantAppPayment(id, pagination)` reads the same records server-side.
...
`[mcp]` The Admin API exposes 59 operations (24 queries, 35 mutations). Family membership by name: **orders** `listOrder`, `listOrderTag`, `listOrderTransactions`, `listOrderSession`, `listAbandonedCheckouts`, `createOrderWithTransactions`, `fulfillOrder`, `cancelFulfillment`, `cancelOrderLine`, `refundOrderLine`, `updateOrderPackageStatus`, `addOrderInvoice`, `removeOrderInvoice`, `downloadOrderInvoice`, `approvePendingOrderTransactions`, `addOrderTimelineEntry`; **products** `listProduct`, `listProductAttribute`, `listProductBrand`, `listCategory`, `createProduct`, `updateProduct`, `deleteProductList`, `addVariantToProduct`, `removeVariantFromProduct`, `updateVariantPrices`, `updateCategory`, `updateProductBrand`; **inventories** `saveVariantStocks`; **customers** `listCustomer`, `listCustomerAttribute`, `updateCustomer`, `addCustomerTimelineEntry`; **campaigns** `createCampaign`, `updateCampaign`, `deleteCampaignList`, `addCouponsToCampaign`; **storefronts** `createStorefrontJSScript`, `updateStorefrontJSScript`, `deleteStorefrontJSScript`; **app / identity (no family)** `getAuthorizedApp`, `getMerchant`, `getMerchantLicence`, `listMerchantAppPayment`, `createMerchantAppPayment`, `saveWebhooks`, `deleteWebhook`, `getAppDemoDay`, `addCustomTimelineEntry`; **settings / location (no family)** `getSalesChannel`, `updateSalesChannel`, `getGlobalTaxSettings`, `listShippingSettings`, `listTaxSettings`, `listCountry`, `listState`, `listCity`, `listDistrict`, `listTown`. An operation name not in this list is app-local (a codegen alias such as `listOrdersForInvoice`) — map it by its underlying query.
...
| R7 | Review paused: "2 adet geliştirme mağazası oluşturmanız ve bu mağazaları uygulamanızın izin verilen mağazalar listesine eklemeniz gerekmektedir" | §1 #5 | (`examples/starter-app`, `dashboard-actions-app`, `with-subscription-app`)
- Admin API schema (live) — ikas admin MCP `https://api.myikas.com/api/v2/admin/mcp` (`list`, `introspect <operation>`)
